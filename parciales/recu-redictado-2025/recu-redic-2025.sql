...
from Mascota m inner join Aplicacion_Vacuna a_v on (m.id_mascota = a_v.id_mascota)
where nombreV = 'Tripe Felina' and precio <10000
/*2*/
Select id_mascota, nombre, especie, raza, f_nac, dni_cliente 
from Mascota m inner join Consulta c on (m.id_mascota = c.id_mascota)
except
Select id_mascota, nombre, especie, raza, f_nac, dni_cliente 
from Mascota m inner join Consulta c on (m.id_mascota=c.id_mascota)
where fecha between '2025-01-01' and '2025-12-31'
/*3*/
select nombre, apellido, telefono, count(m.id_mascota)/*cuento las que hay*/
from Mascota m inner join Cliente c (m.dni = c.dni)
group by c.dni
having count(m.id_mascota) >2 /*mas de 2*/
order by count(m.id_mascota) desc /*lo ordeno por las q hay mas*/
/*4*/
select id_mascota, nombre, especie, raza, f_nac, dni_cliente
from Mascota m inner join Consulta c on (m.id_amscota = c.id_mascota)
